...
void Temperature()
{
  if(put.temperature(percent)>50)
  {
    put.setStopping(coast);
    put.stop();
    Controller1.Screen.print("!!放置器过热!!");
  }     
  if(arm.temperature(percent)>50)
  {
    arm.setStopping(coast);
    arm.stop();
    Controller1.Screen.print("!!手臂过热!!");
  } 
  if(leftsuck.temperature(percent)>50||rightsuck.temperature(percent)>50) 
  {
    ...
    rightsuck.setStopping(coast);
    rightsuck.stop();
    Controller1.Screen.print("!!吸盘过热!!");
  }
  if(leftfront.temperature(percent)>75||leftbehind.temperature(percent)>75
    ||rightfront.temperature(percent)>75||rightbehind.temperature(percent)>75) 
  {
    leftfront.setStopping(coast);
    leftfront.stop();
    leftbehind.setStopping(coast);
    ...
    rightfront.setStopping(coast);
    rightbehind.stop();
    Controller1.Screen.print("!!发动机过热!!");
  }    
}
//驱动底盘开启函数
